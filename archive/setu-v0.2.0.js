(function(ns, undefined) {
    ...
    var __json_content_type__ = 'application/json; charset=UTF-8',
        ...
        __meta_attr_chart__ = 'setu-chart',
        __meta_attributes__ = [ __meta_attr_bind__, __meta_attr_click__,
        ...
        __unsupported_error_msg__ = "Unsupported feature"
    ...
    */
    ...
    function _ajax(method, url, config, success, error) {

        config = config || { }

        config.type = method || 'GET'
        config.async = config.async || true
        config.cache = config.cache || false

        config.url = url

        config.beforeSend = config.beforeSend || function(jqXHR, settings) {
            adapters_run('ajax.beforeSend', jqXHR, settings, this)
        }

        config.success = function(data, textStatus, jqXHR) {
            console.debug('Setu.http ajax $', url, method, textStatus, data)
            if (success) {
                success({
                    data    : data,
                    status  : textStatus,
        ...
        config.error = function(jqXHR, textStatus, errorThrown) {
            console_error('Setu.http ajax !', url, method, textStatus, errorThrown)
            if (error) {
    ...
    function GET(url, config, success, error) {
        return _ajax('GET', url, config, success, error)
    }

    function POST(url, config, success, error) {
        return _ajax('POST', url, config, success, error)
    }

    function PATCH(url, config, success, error) {
        return _ajax('PATCH', url, config, success, error)
    }

    function PUT(url, config, success, error) {
        return _ajax('PUT', url, config, success, error)
    }

    function DELETE(url, config, success, error) {
        return _ajax('DELETE', url, config, success, error)
    }
    ...
    function adapters_run(purpose) {
        if(!SetuAdapters[purpose]) return [ ]
    ...
    function events_fire(type, producer, data) {
        var registry = SetuEventsConsumers[type]
    ...
    var SetuModels = { },
        SetuModelsFilters,
        SetuModelsProcessors,
        SetuModelsConfig,
        SetuNameResolution

    function models_setup(specs, filters, processors, config) {
        ...
        SetuModelsProcessors = processors || { }
    ...
    function models_detail(model, pk, success, error) {
        ...
        var url = model.pathPrefix + '/' + pk + (SetuModelsConfig.trailingSlash ? '/' : '')
    ...
    function instances_onchange(instance, field, oldVal, newVal) {
        ...
        events_fire_for(__event_instance_change__, instance.__key, instance, { field : field, oldVal : oldVal, newVal : newVal })
    }
    ...
    function resources_get(which, needed, success) {
        ...
        which.forEach(function(req) {
            ...
            _resources_get_one(title, resource,
            function(result)
            {
    ...
    function _resources_parse_key(needed, key) {
        ...
        if (match = key.match(__named_resource_regex__)) {
            resource = { title : match[1], model : models_get(match[2]), params : match[3], key : match[2] + '|' + match[3] }
        }
        else if(match = key.match(__resource_regex__)) {
            resource = { title : key, model : models_get(match[1]), params : match[2], key : key }
        }
        else {
            resource = { title : key, model : models_get(key), key : key }
        }
    ...
    function resources_dissect_key(needed, key) {
        ...
        if(spec) {
            return { type : 'api', key : key, title : key, api : spec }
        }
    ...
    function _resources_get_one(title, resource, success) {
        ...
        case __keyword_list__:
            if (resource.value) {
                success({ title : title, value : resource.value })
            } else {
                models_list(resource.model, resource.queryparams,
                function(list)
                {
                    if (SetuModelsProcessors[resource.model.name] &&
                        SetuModelsProcessors[resource.model.name].list &&
                        'function' === typeof(SetuModelsProcessors[resource.model.name].list))
                    {
                        list = SetuModelsProcessors[resource.model.name].list(list)
                    }
                    resource.value = list
                ...
                function(empty)
                {
                    success({ title : title, value : empty })
                })
            }
            break
        case __keyword_detail__:
            if (resource.value) {
                ...
                models_detail(resource.model, resource.pk,
                function(instance)
                {
        ...
        case 'api':
            if (resource.value) {
                ...
                function(e)
                {
                    success({ title : title, value : undefined })
                })
            }
            break
        }
    ...
    function meta_is_scope(element) {
        return (_meta_has_attrs(element)||
                ...
                meta_find_subscopes(element).length)
    }
    ...
    function _meta_has_attrs(element) {
        if (element.hasAttribute) {
            for(var i = 0; i < __meta_attributes__.length; ++i) {
                if (element.hasAttribute(__meta_attributes__[i])) return true
            }
        }
        return false
    }

    function meta_find_subscopes(element) {
        var list = element.querySelectorAll(__meta_attribs_selector__)
        var array = [ ]
        list.forEach(function(e) {array.push(e)})
        return array
    }
    ...
    function meta_parse_click(element) {
        ...
        if (-1 !== [__keyword_detail__, __keyword_update__].indexOf(str)) {
            ...
            return { type : __keyword_list__, modelName : match[2] || match[6], filter : match[3] || match[7] }
        }
        else if (match = str.match(__create_regex__)) {
            return { type : __keyword_create__, modelName : match[2] }
        }
        else return { type : 'generic', code : str }
    }

    function meta_parse_form(form) {

        if (form.hasAttribute(__meta_attr_model__)) {
            return { type : __keyword_model__, model : models_get(form.getAttribute(__meta_attr_model__)) }
        }
            ...
            return { type : __keyword_instance__, resource : resource }
        }
    ...
    function _meta_parse_to_safe(element, attrib) {
        ...
        if (!element.hasAttribute || !element.hasAttribute(attrib)) return [ ]
    ...
    function scopes_show(position) {
        ...
        if (position.index > 0) {
            $(position.parent).children().eq(position.index - 1).after(position.data)
        } else {
            $(position.parent).prepend(position.data)
        }

        var element = $(position.parent).children()[position.index]
    ...
    function _scopes_sibling_copy(element, clone) {
        ...
        var cloneSetu = clone.setu = { needed : { } }
    ...
    function parser_run() {
        ...
        if (SetuParserNewCount) --SetuParserNewCount
    ...
    function parser_eval_scope(element) {
        ...
        var children = meta_find_subscopes(element)
        ...
        if (element.hasAttribute(__meta_attr_include__))
        {
            ...
            GET(element.getAttribute(__meta_attr_include__), { },
            function(response)
            {
                console.group()
        ...
        if (element.hasAttribute(__meta_attr_bind__) && !element.setu.template)
        {
            ...
            if (element.hasAttribute(__meta_attr_loop__))
            {
                ...
                $(element).attr(__meta_attr_loop__, loop)
            }
            else {
                setu.template = element.outerHTML
            }
        ...
        if (element.outerHTML.match(__template_regex__))
        {
            ...
            if(element.innerHTML.match(__template_regex__))
            {
                ...
                children = meta_find_subscopes(element)
            }
        ...
        if (element.hasAttribute(__meta_attr_chart__))
        {
            post_meta_render_chart(element)
        }
    ...
    {
        if(null === form.parentNode) return

        if (form.setuForm) return

        events_register('MetaRender',
        function(form)
        {
    ...
    function post_meta_process() {
        ...
        document.querySelectorAll('form').forEach(function(form) {
            form.querySelectorAll('select').forEach(function(select) {
                var option = select.querySelector('option[setu-selected]')
                if (option) {
                    option.setAttribute('selected', 'selected')
                    option.removeAttribute('setu-selected')
                    select.value = option.value
                }
            })
            form.querySelectorAll('input[setu-checked]').forEach(function(input) {
                input.setAttribute('checked', 'checked')
                input.removeAttribute('setu-checked')
            })
        })
    ...
    function post_meta_render_chart(element) {

        var chart_spec = element.getAttribute(__meta_attr_chart__)

        console.log('post_meta_render_chart @ chart_spec', chart_spec)

        adapters_run('chart.render', chart_spec, element)
    }
    ...
    function binds_setup(element) {
        ...
        if (element.setuBind) return true
    ...
    function _binds_on_detail_change(ignored, resource, data) {
        ...
        if (_binds_handle_queuing(_binds_on_detail_change, ignored, resource, data)) return
    ...
    function observer_monitor_additions() {
        if (SetuAppElement && SetuAppElement.parentNode) {
            SetuAdditionsObserver.observe(SetuAppElement, { childList : true, subtree : true })
        }
    }
    ...
    function config_setup(config) {
        ...
        if (SetuConfig.adapters) {
            SetuConfig.adapters.forEach(function(adapter) {
                plugins_register_adapter(adapter)
            })
        }

        if (SetuConfig.charts) {
            adapters_run('chart.config', SetuConfig.charts)
        }
    ...
    */
    ...
    function django_before_send(ignore, jqXHR, settings, context) {

        if(! /^(GET|HEAD|OPTIONS|TRACE)$/.test(settings.type) && !context.crossDomain) {
            var csrfToken = document.cookie.match(new RegExp('csrftoken=([^;]+)'));
            if(csrfToken) {
                jqXHR.setRequestHeader('X-CSRFToken', csrfToken[1])
    ...
    /**
     * Django Rest Framework Adapters
    */

    function drf_list_output(ignore, response) {
    ...
    */
    ...
    function indicate_ajax_start(form) {
        $(form).find('[type=submit],label[for]').each(function() {
            var html = $(this).html()
            $(this).html('<i class="fa fa-circle-o-notch fa-spin fa-fw"></i>' + html)
        })
    }

    function indicate_ajax_over(form) {
        $(form).find('[type=submit],label[for]').each(function() {
            $(this).find('i.fa.fa-circle-o-notch').remove()
        })
    }

    function finre_form_setup(ignore, form) {

        var finreForm = !$(form).data('finre') ? new finre.forms.Form(form) : $(form).data('finre')

        form.onsubmit = (function(finreForm, form) {
            return function() {
                try {
                    finreForm.controls.forEach(function(control) {
                        $(control.element).change()
                    })
                    if(! finreForm.forceSubmit) {
                        var result = finreForm.onsubmit()
                        if(!result) {
                            finreForm.controls.every(function(control) {
                                if(control.in_error) {
                                    $('html,body').animate({scrollTop: control.getTop() - 40}, 400)
                                    return false
                                }
                            })
                            return false
                    ...
                    indicate_ajax_start(form)
                    instances_save(form.setuInstance,
                    function(instance) {
                        indicate_ajax_over(form)
                        console.log('Setu.adapters.finre $ submit', form, instance)
                        events_fire('FormSuccess', form, instance)
                    },
                    function(empty, e) {
                        indicate_ajax_over(form)
                        console_error('Setu.adpaters.finre ! submit', form)
                        var errorObject = JSON.parse(e.message)
                        events_fire('FormError', form, errorObject)
                    })
                    return false
                } catch(e) {
                    console_info('Setu.adpaters.finre', e.message, e.stack)
                    return false
                }
            }
        })(finreForm, form)

        finreForm.controls.forEach(function(c) {
            c.registerOnvalid(function(instance, control) {
                instance[control.element.name] = control.value()
            }, form.setuInstance)
        })
    }
    ...
    */
    ...
    var __chartjs_type_donut_percentage__ = 'donut-percentage';

    function chartjs_config(ignore, config) {
        for (var key in config) {
            Chart.defaults.global[key] = config[key]
        }
    }

    function chartjs_render(ignore, chart_spec, parent_ctxt) {

        var spec_parts = chart_spec.split(' ')

        if (!spec_parts.length) {
            console_error('Setu.adapters.chartjs ! invalid setu-chart', parent_ctxt.getAttribute(__meta_attr_chart__), parent_ctxt)
            throw new Error(__invalid_meta_msg__)
        }

        var type = spec_parts[0]

        switch(type) {
        case __chartjs_type_donut_percentage__:
            _chartjs_render_donut_percentage(spec_parts, parent_ctxt)
            break
        default:
            console_error('Setu.adapters.chartjs ! unsupported chart type', type, parent_ctxt.getAttribute(__meta_attr_chart__))
            throw new Error(__unsupported_error_msg__)
        }
    }

    var __chartjs_donut_percentage_args__ = [ 'percentage', 'color', 'height', 'width', 'labels' ]

    var __chartjs_labels_regex__ = /\['[a-zA-Z0-9_]+','[a-zA-Z0-9_]+'\]/

    var __chartjs_color_from_pct_regex__ = /from_percentage\((\[0x[0-9a-fA-F][0-9a-fA-F],0x[0-9a-fA-F][0-9a-fA-F],0x[0-9a-fA-F][0-9a-fA-F]\]);(\[0x[0-9a-fA-F][0-9a-fA-F],0x[0-9a-fA-F][0-9a-fA-F],0x[0-9a-fA-F][0-9a-fA-F]\])\)/


    function _chartjs_render_donut_percentage(spec_parts, parent_ctxt)
    {
        var args = { }
        spec_parts.forEach(function(part, idx) {
            if (!idx) return
            if (!part.match(/:/)) {
                console_error('Setu.adapters.chartjs ! invalid chart argument', part, parent_ctxt.getAttribute(__meta_attr_chart__))
                throw new Error(__invalid_meta_msg__)
            }
        ...
        __chartjs_donut_percentage_args__.forEach(function(arg) {
            if (!args[arg]) {
                console_error('Setu.adapters.chartjs ! missing donut percentage chart arg', arg, parent_ctxt.getAttribute(__meta_attr_chart__))
                throw new Error(__invalid_meta_msg__)
            }
            switch(arg) {
            case 'percentage':
                var percentage = parseFloat(evals_parse_expr(parent_ctxt.setu.needed, args[arg]))
                if (isNaN(percentage)) {
                    console_error('Setu.adapters.chartjs ! donut-percentage bad percentage arg', args[arg], parent_ctxt.getAttribute(__meta_attr_chart__))
                    throw new Error(__invalid_meta_msg__)
                }
                percentage = percentage.toFixed(2)
                console.debug('Setu.adapters.chartjs @ donut-percentage percentage', percentage)
                args[arg] = percentage
                break
            case 'color':
                var color_spec = args[arg], color
                if (color_spec.match(/^#/)) {
                    color = color_spec
                }
                else if (color_spec.match(/^from_percentage/)) {
                    var match
                    if(!(match = color_spec.match(__chartjs_color_from_pct_regex__))) {
                        console_error('Setu.adapters.chartjs ! donut-percentage bad from_percentage color spec', args[arg], parent_ctxt.getAttribute(__meta_attr_chart__))
                        throw new Error(__invalid_meta_msg__)
                    }
                    var c1 = eval(match[1]), c2 = eval(match[2]), c = '#'
                    if (undefined === args[arg]) {
                        console_error('Setu.adapters.chartjs ! internal programming error')
                        throw new Error(__internal_error_msg__)
                    }
                    for (var i = 0; i < 3; ++i) {
                        var hex = (Math.round((c2[i] - c1[i]) * args.percentage / 100) + c1[i]).toString(16)
                        if (1 == hex.length) hex = '0' + hex
                        c = c + hex
                    }
                    console.debug('Setu.adapters.chartjs @ donut-percentage color', c)
                }
                args[arg] = c
                break
            case 'height':
                var height = parseInt(args[arg])
                if (isNaN(height)) {
                    console_error('Setu.adapters.chartjs ! donut-percentage bad height spec', args[arg], parent_ctxt.getAttribute(__meta_attr_chart__))
                    throw new Error(__invalid_meta_msg__)
                }
                args[arg] = height
                break
            case 'width':
                var width = parseInt(args[arg])
                if (isNaN(width)) {
                    console_error('Setu.adapters.chartjs ! donut-percentage bad width spec', args[arg], parent_ctxt.getAttribute(__meta_attr_chart__))
                    throw new Error(__invalid_meta_msg__)
                }
                args[arg] = width
                break
            case 'labels':
                var labels = args[arg]
                if (!labels.match(__chartjs_labels_regex__)) {
                    console_error('Setu.adapters.chartjs ! donut-percentage bad labels spec', args[arg], parent_ctxt.getAttribute(__meta_attr_chart__))
                    throw new Error(__invalid_meta_msg__)
                }
                labels = eval(labels)
                console.debug('Setu.adapters.chartjs @ donut-percentage labels', labels)
                args[arg] = labels
                break
            }
        })
        ...
        parent_ctxt.appendChild(canvas)

        canvas = parent_ctxt.querySelector('canvas')
        ...
        new Chart(canvas, {
            ...
            data    : {
                datasets: [
                    {
                        data : [ args.percentage, 100 - args.percentage ],
                        labels  : args.labels,
                        backgroundColor : [ args.color, '#ffffff' ]
                    }
                ]
            }
        })
    ...
    var SetuAdaptersPlugins = {
        Django              : [ [ 'ajax.beforeSend', django_before_send ] ],
        DjangoRestFramework : [ [ 'models.listOutput', drf_list_output ] ],
        Finre               : [ [ 'forms.setup', finre_form_setup ] ],
        Chartjs             : [ [ 'chart.render', chartjs_render ], [ 'chart.config', chartjs_config ] ]
    }

    function plugins_register_adapter(name) {
        SetuAdaptersPlugins[name].forEach(function(registry) {
            adapters_register(registry[0], registry[1])
        })
    }
    ...
    ns.models.NameResolutionDefault = {
        ...
        m2p : function(name) {
            return (
            name.replace(/^([A-Z])/, function(v){ return v.toLowerCase() })
        ...
        p2m : function(name) {
            return (
            name.replace(/^([a-z])/, function(v){ return v.toUpperCase() })
                .replace(/(-[a-z])/g, function(v){ return v.charAt(1).toUpperCase() })
                .replace(/s$/, ''))
        },
        ...
        m2i : function(name) {
            return (
            name.replace(/^([A-Z])/, function(v){ return v.toLowerCase() })
                .replace(/([A-Z])/g, function(v){ return '_' + v.toLowerCase() }))
        },

        i2m : function(name) {
            return (
            name.replace(/^([a-z])/, function(v){ return v.toUpperCase() })
                .replace(/(_[a-z])/g, function(v){ return v.charAt(1).toUpperCase() }))
        },
    ...
    function _application_run(name, settings) {
        ...
        models_setup(settings.models, settings.modelFilters, settings.modelProcessors, settings.config && settings.config.models)
    ...
    function _application_resolve_n_execute() {
        ...
        function() {
            if (SetuCurrentRoute.redirect) {
                ...
                meta_find_subscopes(SetuAppElement).forEach(function(element) {
                    SetuParserQueue.push(element)
                    element.setuQueued = true
                })
    ...
    function _application_load_html() {
        ...
        GET(SetuCurrentRoute.include, { },
        function(response)
        {
            SetuAppEnd = SetuAppElement.firstElementChild

            $(SetuAppEnd).before(response.data)

            if (!meta_find_subscopes(SetuAppElement).length) {
                parser_post_run()
            }
        })
    }
